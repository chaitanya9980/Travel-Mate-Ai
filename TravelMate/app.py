...
def update_review_data(review_id, data):
    """Update review in Firebase or mock database"""
    if FIREBASE_ENABLED and FirebaseDB:
        ref = FirebaseDB._get_ref(f"/reviews/{review_id}")
        if ref:
            ref.update(data)
    if review_id in reviews:
        reviews[review_id].update(data)
...
def delete_notification_data(notification_id):
    """Delete notification from Firebase or mock database"""
    if FIREBASE_ENABLED and FirebaseDB:
        FirebaseDB.delete_notification(notification_id)
    global notifications
    notifications = [n for n in notifications if str(n.get('id')) != str(notification_id)]
...
@app.route('/admin/review/delete/<review_id>', methods=['POST'])
def admin_delete_review(review_id):
    ...
    # Delete from Firebase or mock database
    if FIREBASE_ENABLED and FirebaseDB:
        FirebaseDB.delete_review(review_id)
    if review_id in reviews:
...
def admin_notifications():
    ...
    all_notifications = get_notifications_data()
    # Handle both list (mock) and dict (Firebase) formats
    if isinstance(all_notifications, dict):
        notif_list = list(all_notifications.values())
    else:
...
def admin_delete_notification(notification_id):
    ...
    delete_notification_data(notification_id)
...
def admin_send_notification():
    ...
    if ONESIGNAL_ENABLED and OneSignalManager:
        try:
            OneSignalManager.send_notification_to_all(title, message)
            flash('Notification sent successfully via OneSignal!', 'success')
        except Exception as e:
            flash(f'Notification saved but OneSignal failed: {str(e)}', 'warning')
    else:
        flash('Notification saved successfully! (OneSignal not configured)', 'success')
